...
def compute_team_form_snapshot(df_form, team):
    ...
    team_matches = df_form[(df_form['HomeTeam'] == team) | (df_form['AwayTeam'] == team)].copy()
...
def main():
    ...
    # ----------------- Son 5 Maç Özeti -----------------
    def last5_report(team):
        tm = df_form[(df_form['HomeTeam']==team) | (df_form['AwayTeam']==team)].copy()
        tm = tm.sort_values('Date').tail(5)
        if tm.empty:
            return "   (Veri yok)"
        wins, draws, losses = 0,0,0
        lines = []
        for _, m in tm.iloc[::-1].iterrows():  # en yakın tarih ilk yazılsın
            d = pd.to_datetime(m['Date']).strftime("%d.%m.%Y")
            hg = safe_float(m.get('score.fullTime.home', 0), 0)
            ag = safe_float(m.get('score.fullTime.away', 0), 0)
            venue = 'E' if m['HomeTeam']==team else 'D'
            if m['HomeTeam']==team:
            ...
            lines.append(f"   1. {d} {m['AwayTeam'] if m['HomeTeam']==team else m['HomeTeam']} {res} ({int(hg)}-{int(ag)}) ({venue})")
        header = f"   📈 Form: {wins} Galibiyet, {draws} Beraberlik, {losses} Mağlubiyet"
        return "\n".join([header] + lines[:3])  # en çok 3 satır göster

    print("\n📊 Takım Form Durumları:")
    print("========================================\n")
    print(f"{home_team} Son 5 Maç:")
    print(last5_report(home_team))
    print(f"\n{away_team} Son 5 Maç:")
    print(last5_report(away_team))
